...
- [X] T066 [US1] 连接成功后停止扫描和广播
  - **文件**: `src/remote/screens/DeviceDiscoveryScreen.tsx`
  - **依赖**: T058, T061
  - **描述**: 当用户选择设备并成功建立连接后，立即停止所有发现活动（广播和扫描）
  - **实现要点**:
    1. 在 `handleConnect` 函数中，当连接成功（`session` 存在）时：
       - 调用 `discoveryOrchestrator.stopDiscovery()` 停止发现
       - 更新 `isScanning` 状态为 `false`
       - 更新 `discoveryPhase` 状态为 `'complete'`
    2. 确保在 `router.replace('/remote/control')` 跳转前完成停止操作
    3. 添加日志记录：`[DeviceDiscovery] Stopping discovery due to successful connection`
  - **验收标准**:
    - 用户连接设备后，后台无残留的广播定时器或扫描任务
    - 日志中显示 `[AggregatedDiscovery] Stopping discovery...`
    - 跳转到遥控器界面后，网络流量显著降低（无扫描请求）
...
| 连接时停止发现 | 用户连接成功后，立即停止扫描和广播 | ✅ 已实现 (T066) |
...
- **2025-12-04**: 新增 T066 任务 - 连接成功后停止扫描和广播；更新验收标准表格增加"连接时停止发现"项
