...
- [X] T004 在 `tests/` 下创建测试目录结构：`unit/`, `integration/`, `e2e/`
- [X] T005 在项目 README 或 `specs/001-tv-remote-mvp/quickstart.md` 中补充 `TV_REMOTE_ENV=mock` 启动示例命令
...
- [X] T008 [P] 实现基础设备状态存储：在 `app/remote/services/device-store.ts` 中实现 `DeviceManager` 的默认实现（内存状态 + AsyncStorage 持久化）
...
- [X] T011 建立基础日志服务：在 `app/remote/services/logger.ts` 中实现记录 `LogEvent` 的简单接口（当前可先输出到控制台，保留后续接入远端/本地文件扩展点）
- [X] T012 配置 Jest + React Native Testing Library：在 `jest.config.*` 与 `tests/unit/` 下添加基础配置和示例测试，确认能运行 TypeScript 测试
...
- [X] T013 [P] [US1] 在 `tests/unit/domain/remote-session.test.ts` 中为 `TVSession` 核心状态机与命令发送行为编写单元测试（含成功与失败路径）
- [X] T014 [P] [US1] 在 `tests/integration/us1_single_device_flow.test.ts` 中编写集成测试（可使用 mock 适配器）验证：发现 → 连接 → 发送基础指令 流程

### Implementation for User Story 1 - 抽象层与协议集成

- [X] T015 [P] [US1] 在 `app/remote/protocols/base-adapter.ts` 中实现通用 `PlatformAdapter` 抽象类（含能力声明与错误码规范）
- [X] T016 [P] [US1] 在 `src/remote/protocols/android-tv-adapter.ts` 和 `src/remote/protocols/fire-tv-adapter.ts` 中实现 Android TV / Fire TV 适配器骨架（预留 ADB over TCP 连接与 keyevent 映射）
- [X] T017 [P] [US1] 在 `app/remote/protocols/webos-adapter.ts` 中实现 LG webOS WebSocket 协议适配器骨架
- [X] T018 [P] [US1] 在 `app/remote/protocols/tizen-adapter.ts` 中实现 Samsung Tizen WebSocket 遥控适配器骨架
- [X] T019 [P] [US1] 在 `app/remote/protocols/roku-adapter.ts` 中实现 Roku ECP HTTP 控制适配器骨架
- [X] T020 [US1] 在 `app/remote/services/session-manager.ts` 中实现基于 `TVSession` 抽象的连接管理（含自动重连策略：2s → 4s → 8s，最多 3 次）

- [X] T0161 [P] [US1] 在 `src/remote/services/mdns-discovery.ts` 中实现 Android TV mDNS 设备发现服务

  **目标**：通过 mDNS 协议发现局域网内的 Android TV / Google TV 设备。

  **技术背景**：
  - Android TV 通过 mDNS 广播 `_androidtvremote._tcp` 服务
  - Google TV 设备同时广播 `_googlecast._tcp` 服务
  - 使用 `react-native-zeroconf` 实现 mDNS 发现

  **验收标准**：
  - mDNS 发现可以找到局域网内的 Android TV 设备
  - 设备信息（名称、IP、型号）正确解析
  - 日志中显示 `[mDNS] Found Android TV: <device-name> at <ip>`

- [X] T0162 [P] [US1] 在 `src/remote/services/aggregated-discovery.ts` 中实现聚合发现服务

  **目标**：支持多品牌多协议同时搜索，并行运行所有平台的发现方法。

  **实现要点**：
  1. 并行运行 Roku SSDP、Android TV mDNS、Fire TV DIAL 发现
  2. 合并和去重发现结果
  3. 提供 `discoverAllDevices()` 统一接口
  4. 支持按平台过滤和快速发现模式

- [X] T0191 [P] [US1] 在 `src/remote/services/ssdp-discovery.ts` 中实现 Roku SSDP 设备发现服务

### Implementation for User Story 1 - 设备发现与连接

- [X] T021 [P] [US1] 在 `src/remote/services/aggregated-discovery.ts` 中实现通用发现接口与平台/协议特定发现策略（支持 mDNS/SSDP/HTTP 多协议并行发现）
...
- [X] T028 [US1] 在 `app/remote/services/command-dispatcher.ts` 中实现规范化命令到 `PlatformAdapter` 的映射与错误处理（含不支持命令时的提示/禁用逻辑）
...
- [X] T029 [P] [US2] 在 `tests/unit/services/device-store.test.ts` 中为 `DeviceManager` 的多设备增删改查与激活设备切换编写单元测试
- [X] T030 [P] [US2] 在 `tests/integration/us2_multi_device_switch.test.ts` 中编写集成测试，验证：添加两台设备 → 切换当前控制目标 → US1 遥控界面针对不同设备发送命令

### Implementation for User Story 2 - 多设备数据与逻辑

- [X] T031 [P] [US2] 扩展 `app/remote/services/device-store.ts`：支持持久化最多 10 台设备列表与用户自定义标签，并在达到上限时阻止新增
- [X] T032 [US2] 在 `app/remote/services/device-store.ts` 中实现设备重命名与删除 API，确保不会影响其他设备的会话与存储
- [X] T033 [US2] 在 `app/remote/services/session-manager.ts` 中支持根据 `activeDeviceId` 快速切换当前 `TVSession`（必要时重建或复用会话）

### Implementation for User Story 2 - UI 与交互

- [X] T034 [P] [US2] 在 `app/remote/screens/DeviceManagementScreen.tsx` 中实现设备管理界面（设备列表、添加、重命名、删除）
- [X] T035 [P] [US2] 在 `app/remote/components/DeviceManagementItem.tsx` 中实现支持长按触发重命名/删除动作的列表项组件
- [X] T036 [US2] 在 `app/remote/screens/RemoteControlScreen.tsx` 中集成快速切换当前设备的入口（例如顶部设备选择器），调用 `DeviceManager` 切换 `activeDeviceId`
...
- [X] T037 [P] [US3] 在 `tests/unit/domain/remote-profile.test.ts` 中测试 `RemoteProfile` 与 `TVCapabilities` 的交互：在不同平台能力下按钮可见性与可用性是否符合预期
- [X] T038 [P] [US3] 在 `tests/integration/us3_cross_platform_ui.test.ts` 中编写集成测试：模拟 Android TV 与 Tizen 设备，验证在切换设备时 UI 布局不变但底层命令映射不同

### Implementation for User Story 3 - 统一布局与能力驱动 UI

- [X] T039 [P] [US3] 扩展 `app/remote/domain/default-profile.ts`：为每个 `RemoteButton` 增加基于 `TVCapabilities` 的 `visibilityRule` 与禁用规则
- [X] T040 [P] [US3] 在 `app/remote/components/RemoteButton.tsx` 中实现根据 `visibilityRule` 与当前设备 `TVCapabilities` 动态隐藏/禁用按钮的逻辑
- [X] T041 [US3] 在 `app/remote/screens/RemoteControlScreen.tsx` 中将平台差异全部下沉到抽象层与 `TVCapabilities`，保证布局与交互模式在不同设备间保持一致
- [ ] T042 [US3] 在 `app/remote/protocols/*-adapter.ts` 中补全各平台的能力探测逻辑（填充 `TVCapabilities`），并确保对不支持命令返回 `COMMAND_UNSUPPORTED` 错误码 *(需要真机调试，暂不修改)*
...
- [X] T043 [P] 完善 `specs/001-tv-remote-mvp/quickstart.md`，加入多设备与多平台实测说明
- [X] T044 [P] 在 `tests/unit/` 和 `tests/integration/` 中补充更多错误场景与边界条件测试（如设备离线、网络切换）
- [ ] T045 对 `app/remote/services/session-manager.ts` 与各 `protocols/*-adapter.ts` 进行性能与资源占用优化（如连接复用、避免过度重连）
- [X] T046 对日志内容进行审查，确保不记录敏感个人信息，仅保留必要的调试字段
- [X] T047 在 `app/remote/` 模块中进行必要的代码整理与小范围重构，保持函数长度与复杂度在宪法要求范围内
